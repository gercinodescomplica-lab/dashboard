...
import { Handshake, Sun, Moon } from 'lucide-react';
...
export function ProposalsPageClient({ propostas, managersList }: ProposalsPageClientProps) {
    ...
    return (
        <div className={`min-h-screen flex flex-col transition-colors duration-200 ${lightActive ? 'bg-white text-zinc-900' : 'bg-zinc-950 text-zinc-50'}`}>
            <header className={`flex-none px-6 py-4 border-b flex items-center justify-between transition-colors duration-200 ${lightActive ? 'bg-white border-zinc-200' : 'bg-zinc-950 border-zinc-900/70'}`}>
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-3">
                        <div className={`p-2 rounded-lg border ${lightActive ? 'bg-emerald-50 border-emerald-200' : 'bg-emerald-500/10 border-emerald-500/20'}`}>
                            <Handshake className={`w-5 h-5 ${lightActive ? 'text-emerald-600' : 'text-emerald-400'}`} />
                        </div>
                        <div>
                            <h1 className={`text-lg font-bold leading-tight ${lightActive ? 'text-zinc-900' : 'text-zinc-100'}`}>Propostas</h1>
                            <p className={`text-xs ${lightActive ? 'text-zinc-500' : 'text-zinc-500'}`}>Pipeline comercial — Oportunidades abertas</p>
                        </div>
                    </div>
                </div>
            ...
            <main className="flex-1 flex flex-col p-6 gap-0 overflow-hidden" style={{ height: 'calc(100vh - 73px)' }}>
                <ProposalsTable initialData={propostas} managersList={managersList} readOnly lightActive={lightActive} />
            </main>
        </div>
    );
}
