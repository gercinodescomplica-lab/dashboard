...
import { verifyPipelineKey } from './actions';
import { Input } from '@/components/ui/input';
import { Button } from '@/components/ui/button';
import { Lock } from 'lucide-react';

export function KeyGatePipeline({ children }: { children: React.ReactNode }) {
    ...
    useEffect(() => {
        ...
        const storedKey = localStorage.getItem('PIPELINE_KEY');
        if (storedKey) {
            verifyPipelineKey(storedKey).then(isValid => {
                if (isValid) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setError(false);
        setIsChecking(true);
        const isValid = await verifyPipelineKey(password);
        if (isValid) {
            localStorage.setItem('PIPELINE_KEY', password);
            setIsAuthenticated(true);
        } else {
            setError(true);
        }
        ...
        return (
            <div className="min-h-screen bg-zinc-950 text-white flex items-center justify-center p-4">
                <div className="max-w-md w-full bg-zinc-900 border border-zinc-800 rounded-2xl p-8 shadow-2xl flex flex-col items-center">
                    ...
                    <h1 className="text-2xl font-bold mb-2">Gestão de Produto</h1>
                    <p className="text-zinc-400 text-center mb-8">Esta área é restrita e necessita de chave de acesso (PIPELINE_KEY).</p>
